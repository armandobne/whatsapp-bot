...
# Configurar a API key para o módulo de integração
set_api_key(MONDAY_API_KEY)
...
Você é o assistente virtual da empresa de recrutamento Mais Emprego. 
...
Seu objetivo é ajudar candidatos a enviarem currículos e, após isso, orientá-los sobre a possibilidade de ativar o cadastro por R$10,00 para ter prioridade nas indicações por 30 dias.
...
Você é o assistente de gestão de recrutamento da Mais Emprego.
...
def obter_resposta_chatgpt(mensagem, personalidade=PERSONALIDADE_CANDIDATO):
        ...
        if candidato.get("estagio", 0) >= 4:
            ...
            if candidato.get("monday_id"):
                try:
                    ...
                    sucesso = registrar_pagamento_candidato(
                        candidato.get("monday_id"), 
                    ...
                    if sucesso:
                        ...
                        resposta = "Tivemos um problema ao processar seu pagamento. Por favor, entre em contato com nosso suporte."
                except Exception as e:
...
def processar_texto_candidato(chat_id, texto, candidato, estagio_atual):
    ...
    # Se já enviou currículo mas ainda não ativou
        ...
        if re.search(r"ativar|ativação|como funciona|valor|custo|taxa|pago|r\$|prioridade", texto, re.IGNORECASE) or avaliar_momento_ativacao(chat_id):
            ...
            return (
                ...
                "A ativação do cadastro tem uma taxa única de *R$10,00* (válida por 30 dias). "
    ...
    # Se já recebeu explicação sobre ativação
        ...
        if re.search(r"sim|quero|ativar|como pago|onde pago|pix", texto, re.IGNORECASE):
            ...
            return (
                ...
                "Assim que recebermos, sua ativação será processada imediatamente e "
    ...
    # Se está no processo de pagamento
        ...
        if re.search(r"paguei|fiz.*pix|transferi|enviei", texto, re.IGNORECASE):
            return (
                "Obrigado! Por favor, envie o comprovante do pagamento (pode ser um print ou foto). "
...
def processar_mensagem_empresa(chat_id, nome, mensagem, tipo_mensagem, texto_mensagem):
    ...
    if not empresa:
        ...
        mensagem_boas_vindas = (
            f"Olá! Sou o assistente de recrutamento da Mais Emprego. Bem-vindo ao nosso sistema de publicação de vagas. "
...
def webhook():
    try:
        ...
        if tipo_usuario == "empresa":
            ...
            # Processar mensagem de candidato
